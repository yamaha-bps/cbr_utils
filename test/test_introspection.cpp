...
TEST(Introspection, to_tuple)
{
  ...
  auto b1 = cbr::bind_to_tuple<const cbr::FusionStruct>(fs1);
  ...
  auto a3 = cbr::copy_to_tuple(cbr::FusionStruct{"a1", 0, 1.});
...
TEST(Introspection, StaticForAggregate)
{
  ...
  i = 0;
  fs = cbr::FusionStruct {"a1", 1, 2.};
  cbr::static_for_aggregate(const_cast<const cbr::FusionStruct &>(fs), fun);
  ASSERT_EQ(i, 8);
  ASSERT_EQ(fs.b, 1);
  ASSERT_EQ(fs.c, 2.);

  i = 0;
  cbr::static_for_aggregate(cbr::FusionStruct{"a1", 1, 2.}, fun);
  ASSERT_EQ(i, 8);
}
